...
def run_test(machine, args, test, critteria, log):
    ...
    ref = True
    if not critteria:
        ref = ref_critteria[0] and ref_critteria[1] and ref_critteria[2]
    else:
        for c in critteria:
            ref = ref and ref_critteria[c]
...
def run_tests(machine, args, tests=TESTS):
    ...
    for i, test in enumerate(tests.items()):
        ...
        try:
            ...
            cscore *= len(critteria)
            print("PASS" if cscore else "FAIL")
            total += cscore
...
def main():
    ...
    if args.validation_type == ["all"] or args.validation_type == "all":
        args.validation_type = ["lines", "columns", "regions"]
